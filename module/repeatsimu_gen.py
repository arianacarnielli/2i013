...
class repeatsimu_gen4V4(object):
...
class repeatsimu_gen1V1(object):
    def __init__(self, list_param, ia, trials=20, max_steps=2000):
        self.strategy = DribleIntelligent1v1Strat
        self.trials = trials
        self.max_steps = max_steps
    ...
    def start(self, show = False, verbose = True):
        ...
        for i in range(self.trials):
            if verbose:
                print(" Match",i)
            brasil = SoccerTeam("Brasil")
            brasil.add("self.strategy", self.strategy(accShoot = self.list_param[0], accDrible = self.list_param[1], vit = self.list_param[2], n = self.list_param[3], maxAngle = self.list_param[4], tooFar = self.list_param[5], rSurfBut = self.list_param[6], AngleHyst = self.list_param[7], alpha = self.list_param[8]))
